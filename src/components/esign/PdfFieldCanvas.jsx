...
export default function PdfFieldCanvas({ fileUrl, renderPageOverlay, onPageClick, onReady, className = "" }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const bytes = await fetch(fileUrl).then(r => { if (!r.ok) throw new Error(`Could not fetch document (${r.status})`); return r.arrayBuffer(); });
        if (cancelled) return;
    ...
    <div ref={wrapRef} className={className}>
      ...
      {status === "ready" && pages.map(meta => (
        <div key={meta.index}
          ...
          <canvas ref={el => { canvasRefs.current[meta.index] = el; }} className="block w-full" />
