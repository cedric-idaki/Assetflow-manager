...
const MembersTab = ({ ctx }) => {
  ...
  // Every field in the member form is mandatory — member numbers are the
  // sacco's own numbering scheme, never generated by the system.
  const REQUIRED_FIELDS = [
    ['full_name', 'Full name'],
    ['member_no', 'Member number / ID'],
    ['phone', 'Phone'],
    ['email', 'Email'],
  ...
  const save = async () => {
    ...
    const memberNo = form.member_no.trim();
    ...
    >
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Member number / ID *"><TextInput value={form.member_no} onChange={(e) => set('member_no', e.target.value)} placeholder="e.g. MEM-001" /></Field>
