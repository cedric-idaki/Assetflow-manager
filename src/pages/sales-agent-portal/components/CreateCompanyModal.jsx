...
// (super-admin-assigned) plan. Agents with no plan default to bronze.
...
// ── Main component ────────────────────────────────────────────────────────────
const CreateCompanyModal = ({ isOpen, onClose, agentProfile, prefillLead, onSuccess }) => {
  const { user } = useAuth();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ── Credit the agent's commission for registering this admin ──────────
      ...
      if (agentProfile?.id && commission > 0) {
        try {
          await supabase.from('agent_wallets').insert({
            ...
            tx_type:           'credit',
            description:       `${planKey.charAt(0).toUpperCase() + planKey.slice(1)} plan commission — registered admin ${form.full_name.trim()} (${form.company_name.trim()})`,
          });
        } catch (walletErr) {
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <select value={form.gender} onChange={e => set('gender', e.target.value)}
                        className={ic(errors.gender)} style={{ color: form.gender ? undefined : '#9ca3af' }}>
