            ...
            {preview ? (
              <div className={`${S.panel} overflow-hidden`}>
                <div className="bg-gradient-to-r from-primary to-primary/80 px-5 py-4">
                  ...
                  <p className="text-base font-semibold text-primary-foreground mt-3">{selectedEmp?.full_name || 'Employee'}</p>
                  <p className="text-sm text-primary-foreground/70">{fmtMonth(month)} · {selectedEmp?.department || '—'}</p>
                  <p className="text-xs font-mono text-primary-foreground/60 mt-1 break-all">ID: {selectedEmp?.id || '—'}</p>
                </div>
                <div className={S.body}>
                  ...
                  <button
                    className={`${S.btnSec} w-full justify-center mt-4`}
                  ...
                  >
                    <Icon name="Printer" size={14} color="currentColor" /> Print Payslip
                  </button>
                </div>
              </div>
            ) : (
                        ...
                        <tr key={r.id} className={S.row}>
                          ...
                          <td className={S.td}>
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground border border-border px-2.5 py-1 rounded-lg hover:bg-muted transition-colors"
